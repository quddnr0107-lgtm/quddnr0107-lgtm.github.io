---
layout: post
title: "INTP 공부법 한 줄 처방 - 평생 가는 공부법의 시작"
date: 2027-02-08 10:00:00 +0900
categories: [공부법]
tags: ["MBTI", "공부법", "INTP", "논리술사", "INTP 공부법", "MBTI 공부법", "16유형", "성격별", "oneline"]
description: "INTP에게 공부법은  30대 자격증 준비 박 모 씨(INTP)는 공부를 결심한 뒤 첫 한 달 동안 본 학습을 거의 하지 않았다. 대신 그가 한 일은 '인강 vs 책 단권화 vs 기출 회독 vs 백지 복습 vs Anki vs 코넬 노트'의 메타 분석이었다. 학습법..."
...
INTP가 공부를 지속하려면 "호기심 흐름을 끊지 않으면서 시험 범위를 끝내는 균형"이 필요하다. 매일 학습 시간을 둘로 쪼개라. 70%는 진도 진행(인강·기출), 30%는 깊이 파기 슬롯. 둘 중 하나만 하면 INTP는 결국 무너진다.

시간대별로는 INTP의 피크 시간이 사람마다 크게 다르다. 새벽형·올빼미형 모두 흔하다. 1주일만 자기 집중력을 시간대별로 기록해 보면 패턴이 보인다. 거기에 "깊이 파기 슬롯"을 배치하고 진도 진행은 그 외 시간에 둔다. INTP에게 새벽 5시 기상 챌린지를 권하지 마라.

## 다음 단계

INTP에게 공부법 운영체계의 출발점은 **30일 단위 학습법 로테이션 (인강·단권화·문제풀이·백지)** 입니다. INTP는 같은 자극이 길어지면 인지 효율이 급락한다. 30일씩 학습법을 바꾸면 INTP의 호기심이 학습 동력으로 환원된다. 30일은 데이터를 수집하기엔 짧지 않고 지루해지기엔 길지 않다. 1년이면 4가지 학습법을 자기 몸으로 검증한 셈이 되고, 그 데이터는 어떤 합격...
